...
library(scran)
library(scater)
devtools::load_all("/shares/common/users/m.thompson/scGPS")

#Get the path to the dataset
path_to_data = "/shares/common/users/m.thompson/Datasets/scMapDataset/"

#Read the data
baron_dat <- readRDS(paste0(path_to_data,"baron-human.rds"))
...
baron_dat_exprs <- assays(baron_dat)[["cpm"]]
...
mixedpop <-NewscGPS_SME(ExpressionMatrix = baron_dat_exprs, GeneMetadata = baron_dat_GeneMetaData, CellMetadata = baron_dat_cellnames)
...
#library("WriteXLS")
#WriteXLS(compare_frame, ExcelFileName = "/shares/common/users/m.thompson/BaronSC3/baronscore.xls")

#Save Single Cell Experiment
saveRDS(CORE_cluster_bagging, "Bagging_Baron.rds")
...
#Read the data
sce <- readRDS(paste0(path_to_data,"baron-human.rds"))
...
#Save the results to a spreadsheet (requires perl)
library("WriteXLS")
WriteXLS(compare_frame, ExcelFileName = "/shares/common/users/m.thompson/BaronSC3/baron.xls")

#Save Single Cell Experiment
saveRDS(SC3result, "SC3_baron.rds")

#Remove unwanted data and save
rm(list = setdiff(ls(), c("AdjustedRandIndex_SC3", "time_difference_SC3", "SC3_k_estimate")))
save.image("baron_SC3_gene_filtered.RData")
```
