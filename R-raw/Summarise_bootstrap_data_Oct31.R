...
f_list =list.files(pattern='QCall_DEgenes_forCluster*')
f_read <-lapply(f_list, function(x)readRDS(x))

f_list_MK =list.files(pattern='QCall_KnownMarkers_forCluster*')
f_read_MK <-lapply(f_list_MK, function(x)readRDS(x))


deviance_explained_MK <-c()
for (i in 1:length(f_read_MK)){
    t <- f_read_MK[[i]][2]$Deviance$Deviance
    deviance_explained_MK <-c(deviance_explained_MK,t[length(t)-1])
}

deviance_explained <-c()
for (i in 1:length(f_read)){
    t <- f_read[[i]][2]$Deviance$Deviance
    deviance_explained<-c(deviance_explained,t[length(t)-1])
}
deviance_explained
f_list
deviance_explained_MK
f_list_MK

t_MK <-as.data.frame(cbind(deviance_explained_MK, f_list_MK))
write.table(t_MK, "deviance_explained_KnownMarKers_summary.txt", sep="\t", col.names=T, row.names=F, quote=F)

t_DE <-as.data.frame(cbind(deviance_explained, f_list))
write.table(t_DE, "deviance_explained_DEgenes_summary.txt", sep="\t", col.names=T, row.names=F, quote=F)
#find the difference
as.numeric(as.vector(t_DE$deviance_explained)) -as.numeric(as.vector(t_MK$deviance_explained_MK))
