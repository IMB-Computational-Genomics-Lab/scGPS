...
PCA_scGPS <- function(expression.matrix=NULL, ngenes = 1500, scaling = TRUE, npcs=50){
  ...
  if(scaling == TRUE) {
    pca.result <- stats::prcomp(pca.input.matrix, scale=T)
  } else {
    pca.result <- stats::prcomp(pca.input.matrix, scale=F)
  }
...
#'
CIDR_scGPS <- function(expression.matrix=NULL,ngenes = 1500, scaling = TRUE, theta =0.5, perplexity=30){
...
tSNE_scGPS <- function(expression.matrix=NULL,ngenes = 1500, scaling = TRUE,
  ...
  print("Running tSNE ...")
  tsne <- Rtsne::Rtsne(reducedDat, pca = T, perplexity = perplexity, theta = theta, ignore_duplicates = TRUE)
  return(tsne$Y)
}
