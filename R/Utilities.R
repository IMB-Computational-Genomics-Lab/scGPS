...
#'                     GeneMetadata = day2$dat2geneInfo,
...
plotReduced_scGPS <- function(reduced_dat, color_fac = factor(Sample_id), 
    dims = c(1, 2), dimNames = c("Dim 1", "Dim 2"), palletes = NULL,
    ...
    yaxis <- cowplot::axis_canvas(p, axis = "y", coord_flip = TRUE) + 
        geom_density(data = reduced_dat_toPlot, 
        aes(`Dim 2`, ..count.., fill = color_fac), size = 0.2, alpha = 0.7) +
        coord_flip() + 
        scale_fill_manual(name = "Samples", values = palletes[1:sample_num],
            limits = sort(as.character(as.vector(unique(color_fac)))))
    
    xaxis <- cowplot::axis_canvas(p, axis = "x") + 
        geom_density(data = reduced_dat_toPlot, aes(`Dim 1`, ..count.., 
            fill = color_fac), size = 0.4, alpha = 0.7) + 
        scale_fill_manual(name = "Samples", values = palletes[1:sample_num],
            limits = sort(as.character(as.vector(unique(color_fac)))))
...
#' @param fitType string specifying 'local' or 'parametric' for 
#' DEseq dispersion estimation
...
findMarkers_scGPS <- function(expression_matrix = NULL, cluster = NULL, 
    ...
    for (cl_id in unique(selected_cluster)) {
        ...
        cds = DESeq::estimateDispersions(cds, method = "per-condition",
            fitType = fitType)
        print(
            paste0("Done estimate dispersions. Start nbinom test for cluster ", 
            as.character(cl_id), "..."))
        res1 = DESeq::nbinomTest(cds, "Others", as.character(cl_id))
        print(paste0("Done nbinom test for cluster ",
            as.character(cl_id), " ..."))
        # adjust folchange
        print(paste0("Adjust foldchange by subtracting basemean to 1..."))
        res1 <- mutate(res1, AdjustedFC = (baseMeanB - 1)/(baseMeanA - 1))
        res1 <- mutate(res1,
            AdjustedLogFC = log2((baseMeanB - 1)/(baseMeanA - 1)))
        # order
...
#' clusterProfiler for this analysis type in R. 
#' The function require installation of several databases as described below.
...
#' @param output_path is a string of the path to the location
...
annotate_scGPS <- function(DEgeneList, pvalueCutoff = 0.05, gene_symbol = TRUE,
    ...
    Reactome_pathway_test <- ReactomePA::enrichPathway(gene = 
        convert_to_gene_ID$ENTREZID, pvalueCutoff = 0.05, readable = TRUE)
