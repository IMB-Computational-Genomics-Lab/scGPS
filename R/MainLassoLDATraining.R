...
training_scGPS <- function(genes = NULL, cluster_mixedpop1 = NULL, 
    ...
    subremaining1_indx <- which(cluster_mixedpop1 != c_selectID)  #remaining
    ...
    # fitting with lda, also with cross validation scaled and centered data
    # before training standardizing data (centered and scaled) - this step is
    # not necessary in the function glmnet
    ...
    cvfit = cv.glmnet(as.matrix(dataset[, 
        -which(colnames(dataset) == "Cluster_class")]), y_cat, 
        family = "binomial", type.measure = "class", standardize = TRUE)
    ...
    fit.lda <- train(Cluster_class ~ ., preProcess = NULL, data = dataset, 
        method = "lda", metric = metric, trControl = trainControl,
        na.action = na.omit)
    ...
    dat_DE_Lambda_idx <- which(dat_DE$lambda == round(cvfit$lambda.min, 
        digits = 5))  #glmnet lambda min has 5 decimal digits
    ...
    dat_DE_fm_DE <- dat_DE %>% group_by(Dfd) %>% 
    ...
    cluster_select_indx_Round2 <- subpop1cluster_indx[
        -which(subpop1cluster_indx %in% subpop1_train_indx)]
    
    # subremaining1_train_indx is the index for remaining class, in contrast to 
    # class 1
    ...
    if (length(subremaining1_indx) - length(subremaining1_train_indx) > 
        ...
        cluster_compare_indx_Round2 <- subremaining1_indx[
            -subremaining1_train_order]  
            #use all of the class 2 cells after taken those for training 
    }
    
    # select common genes again because genes with no variation are removed 
    # before standardization
    genes_in_trainset <- row.names(cvfit$glmnet.fit$beta)
    genes_in_trainset_idx <- match(genes_in_trainset, 
        rownames(mixedpop1[genes_in_both_idx1, ]))
    ...
    temp_S2 <- mixedpop1[genes_in_trainset_idx_ordered, 
    ...
    # check how many genes in gene_cvfit but not in mixedpop2 (should be 0, 
    # as this is checked in the training step, if not add random genes)
    ...
    if (to_add > 0) {
        ...
        cvfitGenes_idx <- sample(cvfitGenes_idx, length(gene_cvfit), 
            replace = FALSE)
    }
    
    # predictor_S2_temp <- ori_dat_2[c(to_add_idx, cvfitGenes_idx), 
    # cluster_select]
    ...
    # training set), using the initial cellNames_cluster
    ...
    cluster_select_predict <- subset(compare, 
        ...
        compare$predict_label == c_selectID) | (as.numeric(compare[, 2]) != 
...
predicting_scGPS <- function(listData = NULL, cluster_mixedpop2 = NULL, 
    ...
    for (clust in unique(my.clusters)) {
        ...
        # convert table() to 2x2 dataframe, it will always have 2 variable 
        # names: $name, Freq
        ElasticNet_cluster_idx <- which(ElasticNet_result[, 1] == c_selectID)
        predict_clusters_ElasticNet <- as.numeric(ElasticNet_result[
