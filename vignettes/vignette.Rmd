...
#To install from github (Depending on the configuration of the local omputer or HPC, possible C++ compilation errors may happen - see installation troubleshoot below) 
devtools::install_github("IMB-Computational-Genomics-Lab/scGPS")

#for compilation trouble shoot, manual download and install can be done by:
#download the package from github
git clone https://github.com/IMB-Computational-Genomics-Lab/scGPS
#check in scGPS/src if the compiled scGPS.so file exists and delete it 
#then manually install and compile in R 
install.packages("scGPS", repos = NULL, type="source")

#alternatively, with the scGPS as the R working directory, you can use to load the package: 
devtools::load_all()
